...
local function TestAOI()
	local aoi = aoi:new(10, 10, 10)
	local inArea = aoi:Enter(1, 0, 0, 0, 1, 1, 1)
	assert(checkAOI(inArea, {1}))
	inArea = aoi:Enter(2, 0, 1, 0, 1, 1, 1)
	assert(checkAOI(inArea, {1, 2}))
	inArea = aoi:Enter(3, 1, 0, 0, 1, 1, 1)
	assert(checkAOI(inArea, {1, 2, 3}))
	inArea = aoi:Enter(4, 1, 1, 0, 1, 1, 1)
	assert(checkAOI(inArea, {1, 2, 3, 4}))
	
	inArea = aoi:Enter(5, 1, 2, 0, 1, 1, 1)
	assert(checkAOI(inArea, {5, 4, 2}))
	inArea = aoi:Enter(6, 2, 1, 0, 1, 1, 1)
	assert(checkAOI(inArea, {3, 4, 5, 6}))
	inArea = aoi:Enter(7, 0, 2, 0, 1, 1, 1)
	assert(checkAOI(inArea, {2, 4, 5, 7}))
	inArea = aoi:Enter(8, 2, 0, 0, 1, 1, 1)
	assert(checkAOI(inArea, {3, 4, 6, 8}))
	
	inArea = aoi:getAOIArea(8)
	assert(checkAOI(inArea, {3, 4, 6, 8}))
	inArea = aoi:getAOIArea(8, 2, 2, 2)
	assert(checkAOI(inArea, {1, 2, 3, 4, 5, 6, 7, 8}))
	
	inArea = aoi:Leave(8)
	assert(checkAOI(inArea, {3, 4, 6, 8}))
	
	local inArea, outArea, newArea  = aoi:Move(1, 2, 0, 0)
	assert(checkAOI(inArea, {3, 4, 6, 1}))
	assert(checkAOI(outArea, {2}))
	assert(checkAOI(newArea, {6}))	
	inArea, outArea, newArea  = aoi:Move(1, 2, 0, 2)
	assert(checkAOI(inArea, {1}))
	assert(checkAOI(outArea, {3, 4, 6}))
	assert(checkAOI(newArea, {}))	
	inArea, outArea, newArea  = aoi:Move(1, 2, 0, 1)
	assert(checkAOI(inArea, {3, 4, 6, 1}))
	assert(checkAOI(outArea, {}))
	assert(checkAOI(newArea, {3, 4, 6}))
end
