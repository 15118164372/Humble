...
{
    if (iLens > sizeof(m_acZero))
    {
        H_Printf("skip write lens %d must less than %d.", (int)iLens, (int)sizeof(m_acZero));
        return;
    }
...
H_Binary& CBinary::getString(void)
{
...
H_Binary& CBinary::getLByte(const unsigned int iLens)
{
    m_stVal.iLens = H_INIT_NUMBER;
    m_stVal.pBufer = NULL;
...
H_Binary& CBinary::readLine(void)
{
    m_stVal.iLens = H_INIT_NUMBER;
    m_stVal.pBufer = NULL;
