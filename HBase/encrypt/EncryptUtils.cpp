...
std::string CEnUtils::zEncode(const char *pszVal, const size_t iLens)
{
    int iErr(Z_OK);
...
std::string CEnUtils::zDecode(const char *pszVal, const size_t iLens)
{
    int iErr(Z_OK);
    size_t iCount(H_INIT_NUMBER);
    char acBuffer[H_ONEK];
...
std::string CEnUtils::uEncode(const char *pszVal, const size_t iLens)
{
    std::string strTemp;
...
std::string CEnUtils::uDecode(const char *pszVal, const size_t iLens)
{
    std::string strTemp;
...
static const unsigned short ausCRC16[256] = {
...
unsigned short CEnUtils::crc16(const char *pszKey, const size_t iLens)
{
...
static const unsigned int auiCRC32[256] = {
...
unsigned int CEnUtils::crc32(const char *pszKey, const size_t iLens)
{
    ...
    while (iKeyLens--)
    {
        uiCRC = auiCRC32[(uiCRC ^ *pVal++) & 0xFF] ^ (uiCRC >> 8);
    }
...
std::string CEnUtils::b64Encode(const char *pszData, const size_t iLens)
{
    size_t i;
    std::string strOut;
...
std::string CEnUtils::b64Decode(const char *pszData, const size_t iLens)
{
    size_t t, x, y, z;
    unsigned char c;
    size_t g(3);
...
std::string CEnUtils::md5Str(const char *pszVal, const size_t iLens)
{
    char digestStr[33];
