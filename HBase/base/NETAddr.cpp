...
int CNETAddr::setRemoteAddr(const H_SOCK &fd)
{
    ...
    sockaddr stAddr;
    socklen_t iSockStorLens((socklen_t)sizeof(stAddr));
    H_Zero(&stAddr, iSockStorLens);

    Clear();
    int iRtn(getpeername(fd, &stAddr, &iSockStorLens));
    if (H_RTN_OK != iRtn)
    {
        iRtn = H_SockError();
...
int CNETAddr::setLocalAddr(const H_SOCK &fd)
{
    ...
    Clear();
    int iRtn(getsockname(fd, &stAddr, &iSockStorLens));
    if (H_RTN_OK != iRtn)
    {
        iRtn = H_SockError();
