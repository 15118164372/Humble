...
void CSHA1::Update(const char* pszData, unsigned int uiLen)
{
    const UINT_8* pbData = (const UINT_8*)pszData;
    UINT_32 uLen(uiLen);
...
bool CSHA1::HashFile(const TCHAR* tszFileName)
{
	...
	while(true)
	{
		...
		if(uRead > 0)
			Update((const char*)pbData, static_cast<UINT_32>(uRead));
...
void CSHA1::Final()
{
	...
	Update((const char*)"\200", 1);

	while((m_count[0] & 504) != 448)
		Update((const char*)"\0", 1);

	Update((const char*)pbFinalCount, 8); // Cause a Transform()
