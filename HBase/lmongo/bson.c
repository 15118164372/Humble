...
lreplace(lua_State *L) {
	...
	case BSON_INT32: {
		...
		int32_t i = (int32_t)lua_tointeger(L,3);
		write_int32(&b, i);
		break;
	}
	case BSON_INT64: {
...
init_oid_header() {
	...
	oid_counter = (uint32_t)(h ^ (uint32_t)time(NULL) ^ (uintptr_t)&h);
}
