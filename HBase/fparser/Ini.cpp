...
{   
    std::string strTmp(CUtils::frontOfFlag(str, "="));
...
void CIniFile::setIntValue(const char *pszNode, const char *pszKey, 
    int iValue)
{
    setStringValue(pszNode, pszKey, CUtils::toString(iValue).c_str());
}

void CIniFile::setFloatValue(const char *pszNode, const char *pszKey, 
    double dValue)
{
    setStringValue(pszNode, pszKey, CUtils::toString(dValue).c_str());
}
